...
export const CheckoutForm = ({ total_amount, cart}) => {
  ...
  async function handleSubmit(e) {
    ...
    const customerData = await supabase.from("customers").upsert(formData).select();
    let customer = customerData.data[0];
    const orderData = await supabase
      .from("orders")
      .upsert({ customer_id: customer.id, value: total_amount }).select();
    let order = orderData.data[0];

    cart.forEach(async (el) => {
      await supabase
        .from("order_product")
        .insert({ order_id: order.id, product_id: el.id, quantity:el.quantity});
    });
  }
